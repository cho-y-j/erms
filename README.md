...
npm install
```
 PowerShell의 실행 정책 때문에 npm.ps1 스크립트 실행이 차단되어 발생한 오류입니다. (PSSecurityException / UnauthorizedAccess)
해결 방법 (아무거나 한 가지 선택)
현재 PowerShell 세션에서만 일시적으로 허용:
현재 사용자에 한해 영구 허용(권장):
관리자 권한 PowerShell에서 설정(필요시):
PowerShell 대신 명령 프롬프트(cmd)에서 실행:
PowerShell에서 npm.cmd를 직접 호출:
추가 확인:
정책 변경 후 터미널을 새로 열고 다시 시도하세요.
...
NEXT_PUBLIC_SUPABASE_URL=https://iuvmwnumwpqcdcovooln.supabase.co
NEXT_PUBLIC_SUPABASE_ANON_KEY=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Iml1dm13bnVtd3BxY2Rjb3Zvb2xuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjAzOTYxNjEsImV4cCI6MjA3NTk3MjE2MX0.zoxTYHpr-efhGb-hM95ZoeaPFvK06_AeXUwJBKNpRnU

# Supabase Access Token (for MCP)
SUPABASE_ACCESS_TOKEN=sbp_b67a1f2892eaa79e6b1d35cd7098866b9f2461ae

# Database Configuration
NEXT_PUBLIC_DATABASE_URL=https://iuvmwnumwpqcdcovooln.supabase.co
```
